...
class Trade:
    def __init__(self):
        ...
        TEST_KEY = 'HUni4emxCgKGapEmyVUNn4dwQOvmcxT2dJhVBDB4QRfXeqZfjJArNNvJotVooq2t' #os.getenv('TEST_KEY')
        TEST_SECRET = 'EkjSgtZbVZ3pg3h5rpKMg7WQguAkX6Qq2Vqc2nLne7yoizCUtoaNTXtK14diiYOg' #os.getenv('TEST_SECRET')
        ...
        self.time_frame="1m" # An extremely volatile timeframe may produce worse results
        ...
        # if not self.live:
        # TESTNET
        self.client_testnet = Spot(base_url='https://testnet.binance.vision', key=TEST_KEY, secret=TEST_SECRET)
        # else:
        # prod
        self.client_prod = Spot(key=LIVE_KEY, secret=LIVE_SECRET)
        self.client_simu = Spot(base_url='http://127.0.0.1', key=TEST_KEY, secret=TEST_SECRET)
        print(f"UNIX Timestamp: {self.client_prod.time()['serverTime']}")
        ...
        while True:
            try:
                ...
                print("Something went wrong in the main loop, retrying in 5 seconds...", e)
                traceback.print_exc()
                if not self.live:
                    # TESTNET
    ...
    def init_trade(self):
        latest_klines = self.client_prod.klines(self.pair, self.time_frame, limit=2)
        ...
        if (time_check > self.latest_time or break_low or take_profit or not self.holding or trailing_stop) and not self.executing:
            ...
            raw_kline = self.client_prod.klines(self.pair, self.time_frame, limit=self.max_df_length)
    ...
    def market_buy(self, pair, quantity):
        ...
        response = self.client_simu.new_order(**buy)
        self.holding = True
        return response
    ...
    def macd_strategy(self, series, index, row):
        ...
        if self.balances == None:
            self.balances = self.client_simu.account()['balances']
            print(self.balances)
        self.holding = False
        ...
        #print("macd_2: ", macd_2)
        #print("macd_1: ", macd_1)
        #print("macd_current: ", macd_current)
        print("SMA: ", row['SMA'], "current: ", current)
        # if macd_2 <= macd_1 and macd_1 <= macd_current and current > float(row['SMA']):
        if current > float(row['SMA']):
            self.bullish = True
        ...
        if index_floor>0 and not self.holding:
            ...
            if self.bullish and open_1 < current and low_1 < float(row['Low']) and not (self.trailing_stop > 0 and (float(row['High']) - float(row['Low']))/float(row['High']) > self.trailing_stop):
                ...
                try:
                    self.client_simu.cancel_open_orders(self.pair)
                except  Exception as e :
                    print(f"[{int(self.latest_time)}] No open orders found.", e)
                    traceback.print_exc()
                # Buy at market price
                ...
                try:
                    ...
                    held = 0
                    print(f"[{int(self.latest_time)}] ERROR CAUGHT: {float(self.buy['executedQty'])}", e)
                    traceback.print_exc()
                    self.balances = self.client_simu.account()['balances']
                    for balance in self.balances:
                        if balance['asset'] == self.symbol:
        ...
        # Sell logic
            ...
            if (not self.bullish and open_2>=close_1) or low_1 > current or (self.take_profit > 0 and current > self.take_price) or (self.trailing_stop > 0 and (float(row['High']) - current)/float(row['High']) > self.trailing_stop):
                ...
                try:
                    ...
                    print(f"[{int(self.latest_time)}] No open orders.", e)
                    traceback.print_exc()
                ...
                try:
                    ...
                    held = 0
                    print(f"[{int(self.latest_time)}] ERROR CAUGHT: MARKET SELL", e)
                    traceback.print_exc()
                    self.balances = self.client_simu.account()['balances']
                    for balance in self.balances:
                        if balance['asset'] == self.symbol:
