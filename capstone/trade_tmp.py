...
class Trade:
    ...
    def init_trade(self):
        ...
        trailing_stop = self.trailing_stop > 0 and self.holding and (float(latest[2]) - float(latest[4])) / float(
            latest[2]) > self.trailing_stop
        ...
        if (time_check > self.latest_time or break_low or take_profit or not self.holding or trailing_stop) and not self.executing:
            ...
            kline_data = pd.DataFrame(raw_kline,
                                      columns=['Date', 'Open', 'High', 'Low', 'Close', 'Volume', 'CloseTime', 'QAV',
                                               'Trades', 'bVolBase', 'bVolQuote', 'Ignore'])
            kline_data['MACD'] = self.macd(kline_data['Close'].astype(float), window_slow=21, window_fast=8,
    ...
    def macd_strategy(self, series, index, row):
        ...
        for balance in balances:
            if balance['asset'] == self.symbol:
                if float(balance[
                             'locked']) * current >= self.min_notional * 1.005:  # add room for price movements and comissions
                    if self.new_candle:
        ...
        if index_floor > 0 and not self.holding:
            ...
            if self.bullish and open_1 < current and low_1 < float(row['Low']) and not (
                    self.trailing_stop > 0 and (float(row['High']) - float(row['Low'])) / float(
                    row['High']) > self.trailing_stop):
                ...
                try:
                    self.stop = self.stop_limit_sell(self.pair, float(self.buy['executedQty']), self.buy_price,
                                                     self.risk_tolerance, self.risk_tolerance * 2)
                except:
                    ...
                    self.stop = self.stop_limit_sell(self.pair, held, self.buy_price, self.risk_tolerance,
        ...
        # Sell logic
            ...
            if (not self.bullish and open_2 >= close_1) or low_1 > current or (
                    self.take_profit > 0 and current > self.take_price) or (
                    self.trailing_stop > 0 and (float(row['High']) - current) / float(
                    row['High']) > self.trailing_stop):
                # Clear any open orders on the pair being traded (cancels any leftovers stop loss limit orders)
                try:
                    self.client.cancel_open_orders(self.pair)
